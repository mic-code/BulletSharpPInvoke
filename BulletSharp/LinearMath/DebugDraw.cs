...
namespace BulletSharp
{
	public abstract class DebugDraw : IDisposable
	{
		...
		public virtual void DrawCapsule(double radius, double halfHeight, int upAxis, ref Matrix transform, ref Vector3d color)
		{
			...
			childTransform.Position = Vector3d.TransformCoordinate(capStart, transform);
			DrawSphere(radius, ref childTransform, ref color);

			childTransform.Position = Vector3d.TransformCoordinate(capEnd, transform);
			DrawSphere(radius, ref childTransform, ref color);

			// Draw some additional lines
			Vector3d start = transform.Position;
