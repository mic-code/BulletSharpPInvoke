...
namespace BulletSharp
{
    ...
    public class RaycastVehicle : IAction
	{
        ...
        Vector3d[] forwardWS = new Vector3d[0];
        Vector3d[] axle = new Vector3d[0];
        ...
        static RaycastVehicle()
        {
            using (var ci = new RigidBodyConstructionInfo(0, null, null))
            {
                ...
                fixedBody.SetMassProps(0, Vector3d.Zero);
            }
        }
        ...
        public WheelInfo AddWheel(Vector3d connectionPointCS, Vector3d wheelDirectionCS0, Vector3d wheelAxleCS, double suspensionRestLength, double wheelRadius, VehicleTuning tuning, bool isFrontWheel)
        {
        ...
        double CalcRollingFriction(RigidBody body0, RigidBody body1, Vector3d contactPosWorld, Vector3d frictionDirectionWorld, double maxImpulse)
        {
            ...
            Vector3d rel_pos1 = contactPosWorld - body0.CenterOfMassPosition;
            Vector3d rel_pos2 = contactPosWorld - body1.CenterOfMassPosition;

            Vector3d vel1 = body0.GetVelocityInLocalPoint(rel_pos1);
            Vector3d vel2 = body1.GetVelocityInLocalPoint(rel_pos2);
            Vector3d vel = vel1 - vel2;

            double vrel;
            Vector3d.Dot(ref frictionDirectionWorld, ref vel, out vrel);
        ...
        Vector3d magenta = new Vector3d(1, 0, 1);
        public void DebugDraw(DebugDraw debugDrawer)
        {
            for (int v = 0; v < NumWheels; v++)
            {
                ...
                Vector3d wheelPosWS = transform.Position;
                ...
                Vector3d to1 = wheelPosWS + axle;
                Vector3d to2 = GetWheelInfo(v).RaycastInfo.ContactPointWS;
        ...
        private double RayCast(WheelInfo wheel)
        {
            ...
            Vector3d rayvector = wheel.RaycastInfo.WheelDirectionWS * raylen;
            ...
            if (obj != null)
            {
                ...
                double denominator = Vector3d.Dot(wheel.RaycastInfo.ContactNormalWS, wheel.RaycastInfo.WheelDirectionWS);

                Vector3d chassis_velocity_at_contactPoint;
                Vector3d relpos = wheel.RaycastInfo.ContactPointWS - RigidBody.CenterOfMassPosition;

                chassis_velocity_at_contactPoint = RigidBody.GetVelocityInLocalPoint(relpos);

                double projVel = Vector3d.Dot(wheel.RaycastInfo.ContactNormalWS, chassis_velocity_at_contactPoint);
        ...
        private void ResolveSingleBilateral(RigidBody body1, Vector3d pos1, RigidBody body2, Vector3d pos2, double distance, Vector3d normal, ref double impulse, double timeStep)
        {
            ...
            Vector3d rel_pos1 = pos1 - body1.CenterOfMassPosition;
            Vector3d rel_pos2 = pos2 - body2.CenterOfMassPosition;

            Vector3d vel1 = body1.GetVelocityInLocalPoint(rel_pos1);
            Vector3d vel2 = body2.GetVelocityInLocalPoint(rel_pos2);
            Vector3d vel = vel1 - vel2;
            ...
            Vector3d m_aJ = Vector3d.TransformCoordinate(Vector3d.Cross(rel_pos1, normal), world2A);
            Vector3d m_bJ = Vector3d.TransformCoordinate(Vector3d.Cross(rel_pos2, -normal), world2B);
            Vector3d m_0MinvJt = body1.InvInertiaDiagLocal * m_aJ;
            Vector3d m_1MinvJt = body2.InvInertiaDiagLocal * m_bJ;
            double dot0, dot1;
            Vector3d.Dot(ref m_0MinvJt, ref m_aJ, out dot0);
            Vector3d.Dot(ref m_1MinvJt, ref m_bJ, out dot1);
            double jacDiagAB = body1.InvMass + dot0 + body2.InvMass + dot1;
            double jacDiagABInv = 1.0f / jacDiagAB;

            double rel_vel;
            Vector3d.Dot(ref normal, ref vel, out rel_vel);
        ...
        public void UpdateFriction(double timeStep)
        {
            ...
            for (int i = 0; i < NumWheels; i++)
            {
                ...
                if (groundObject != null)
                {
                    ...
                    Vector3d surfNormalWS = wheel.RaycastInfo.ContactNormalWS;
                    double proj;
                    Vector3d.Dot(ref axle[i], ref surfNormalWS, out proj);
                    axle[i] -= surfNormalWS * proj;
                    axle[i].Normalize();

                    Vector3d.Cross(ref surfNormalWS, ref axle[i], out forwardWS[i]);
            ...
            for (int i = 0; i < NumWheels; i++)
            {
                ...
                Vector3d rel_pos = wheel.RaycastInfo.ContactPointWS -
                ...
                if (sideImpulse[i] != 0)
                {
                    ...
                    Vector3d rel_pos2 = wheel.RaycastInfo.ContactPointWS -
                        groundObject.CenterOfMassPosition;


                    Vector3d sideImp = axle[i] * sideImpulse[i];
                    ...
                    Vector3d vChassisWorldUp = new Vector3d(
                    ...
                    double dot;
                    Vector3d.Dot(ref vChassisWorldUp, ref rel_pos, out dot);
        ...
        public void UpdateVehicle(double step)
        {
            ...
            if (Vector3d.Dot(forwardW, RigidBody.LinearVelocity) < 0)
            {
            ...
            for (int i = 0; i < wheelInfo.Length; i++)
            {
                ...
                Vector3d impulse = wheel.RaycastInfo.ContactNormalWS * suspensionForce * step;
                Vector3d relpos = wheel.RaycastInfo.ContactPointWS - RigidBody.CenterOfMassPosition;
            ...
            {
                WheelInfo wheel = wheelInfo[i];
                Vector3d relpos = wheel.RaycastInfo.HardPointWS - RigidBody.CenterOfMassPosition;
                Vector3d vel = RigidBody.GetVelocityInLocalPoint(relpos);
                ...
                if (wheel.RaycastInfo.IsInContact)
                {
                    ...
                    double proj = Vector3d.Dot(fwd, wheel.RaycastInfo.ContactNormalWS);
                    fwd -= wheel.RaycastInfo.ContactNormalWS * proj;

                    double proj2;
                    Vector3d.Dot(ref fwd, ref vel, out proj2);
        ...
        public void UpdateWheelTransform(int wheelIndex, bool interpolatedTransform)
        {
            ...
            Vector3d up = -wheel.RaycastInfo.WheelDirectionWS;
            Vector3d right = wheel.RaycastInfo.WheelAxleWS;
            Vector3d fwd = Vector3d.Cross(up, right);
            fwd.Normalize();
            ...
            transform.Position = wheel.RaycastInfo.HardPointWS + wheel.RaycastInfo.WheelDirectionWS * wheel.RaycastInfo.SuspensionLength;
        ...
        void UpdateWheelTransformsWS(WheelInfo wheel, bool interpolatedTransform)
        {
            ...
            wheel.RaycastInfo.HardPointWS = Vector3d.TransformCoordinate(wheel.ChassisConnectionPointCS, chassisTrans);
            Matrix chassisTransBasis = chassisTrans.Basis;
            wheel.RaycastInfo.WheelDirectionWS = Vector3d.TransformCoordinate(wheel.WheelDirectionCS, chassisTransBasis);
            wheel.RaycastInfo.WheelAxleWS = Vector3d.TransformCoordinate(wheel.WheelAxleCS, chassisTransBasis);
        }
	}
    ...
    public class DefaultVehicleRaycaster : IVehicleRaycaster
    {
        ...
        public object CastRay(ref Vector3d from, ref Vector3d to, VehicleRaycasterResult result)
        {
            ...
            using (var rayCallback = new ClosestRayResultCallback(ref from, ref to))
            {
                ...
                if (rayCallback.HasHit)
                {
                    ...
                    if (body != null && body.HasContactResponse)
                    {
                        ...
                        Vector3d hitNormalInWorld = rayCallback.HitNormalWorld;
